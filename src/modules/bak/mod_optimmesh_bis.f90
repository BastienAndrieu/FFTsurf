...
contains
  ...
  subroutine optim_jiao( &
    ...
    real(kind=fp)                          :: uvtmp(2,2), xyztmp(3), dxyz(3), dij(3), duvtmp(2,2)
    ...
    integer                                :: ipass, ivert, jvert, ivar, ihedg(2), jhedg(2), iface, jface
          ...
          case (2) ! -------------------------------------------------------
             ...
             adjacent_verts : do
                ...
                if ( mesh%typ(jvert) == 2 .and. jface /= iface ) then
                   ...
                   dij = mesh%xyz(:,jvert) - mesh%xyz(:,ivert)
                   if ( dot_product(dxyz, dij) > 0._fp ) then
                      do ivar = 1,2 ! <---------------------+
                         call evald1( &                     !
                              tng(:,ivar), &                !
                              brep%faces(jface)%surface, &  !
                              mesh%uv(:,1,jvert), &         !
                              ivar )                        !
                      end do ! <----------------------------+
                      call solve_NxN( &
                           ...
                           matmul(transpose(tng), dxyz - dij), &
                           singular )
                      uvtmp(:,1) = mesh%uv(:,1,jvert) + duvtmp(:,1)
                      if ( maxval(abs(uvtmp(:,1))) < 1._fp + EPSuv ) then
                         idsnew(ivert) = jface
