...
contains
  ...
  subroutine init_from_mesh( &
    ...
    integer                                                :: surfpair(2), order(2)
    integer, allocatable                                   :: polyverts(:)
    integer                                                :: npolyverts
       ...
       do
          if ( .not.visited(ihedg(1),ihedg(2)) .and. &
             ...
             if ( surfpair(2) < surfpair(1) ) then
                order = [2,1]
                !polyverts(1:npolyverts) = polyverts(npolyverts:1:-1)
             else
                order = [1,2]
             end if
             do isurf = 1,2
                if ( surfpair(order(isurf)) > 0 ) curve%surf(isurf)%ptr => surf(surfpair(order(isurf)))
             end do
             PRINT *,'SURFACES :',surfpair(order)
             !                                                                       !
             curve%isplit(2,1:2) = [1,npolyverts]
             curve%isplit(1,1:2) = curve%isplit(1,order)
             ...
             curve%polyline%xyz(1:3,1:npolyverts) = mesh%xyz(1:3,polyverts(1:npolyverts))
             curve%polyline%uv(1:2,1:2,2:npolyverts-1) = &
                  mesh%uv(1:2,order,polyverts(2:npolyverts-1))
             !
             ...
             do iendpoint = 1,2
                ...
                jpv = ipv + (-1)**(iendpoint + 1) ! nearest interior polyline vertex
                incident_surfaces : do isurf = 1,2
                   if ( interdata%points(iip)%npos > 0 ) then
                      ...
                      do ipos = 1,interdata%points(iip)%npos
                         if ( associated(pos%surf, curve%surf(isurf)%ptr) ) then
                            curve%polyline%uv(1:2,order(isurf),ipv) = pos%uv
                   ...
                   pos%uv = curve%polyline%uv(1:2,order(isurf),jpv)
                   !PRINT *,'BEFORE :',pos%uv
                   call projection_surface( &
                        ...
                        xyzproj )
                   !PRINT *,'AFTER :',pos%uv
                   !PRINT *,'|DELTA| =',NORM2(pos%uv - mesh%uv(1:2,isurf,polyverts(jpv)))
                   !PRINT *,'|DELTA| =',NORM2(pos%uv - curve%polyline%uv(1:2,order(isurf),jpv))
                   !IF ( IENDPOINT == 1 ) THEN
                   !   PRINT *, ipv, jpv
                   !ELSE
                   !   PRINT *, jpv, ipv
                   !END IF
                   !PRINT *,'DUV_DS =',REAL((-1)**iendpoint, KIND=FP) * (pos%uv - curve%polyline%uv(1:2,order(isurf),jpv))
                   IF ( STATPROJ > 0 ) THEN
             ...
             do lvert = 2,npolyverts-1
                ...
                IF ( MAX(norm2(mesh%xyz(:,kvert) - xyzverif(:,1)), &
                   ...
                   PRINT *,'I =', kVERT,', ERR =', &
                        norm2(mesh%xyz(:,kvert) - xyzverif(:,1)), norm2(mesh%xyz(:,kvert) - xyzverif(:,2))
                END IF
             end do
             !
