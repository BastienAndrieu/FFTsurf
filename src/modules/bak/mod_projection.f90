       ...
       wires : do iwire = 0,brep%faces(ifacetmp)%ninner
          ...
          halfedges : do
             ...
             if ( ninter > 0 ) then
                ...
                !if ( lamb(1,iinter)*norm2(duvtmp) > EPSuv .or. &
                !     is_in_open_interval(w*norm2(vec), 0._fp, 1._fp, EPSuv) ) then
                if ( is_in_open_interval(w*norm2(vec), 0._fp, norm2(vec), EPSuv) ) then
                   ! relax to the exact intersection (pick the closest poyline point)
                ...
                IF ( DEBUG ) THEN
                   ...
                   PRINT *,'DUV =',duvtmp
                END IF
                ! the current point is now exactly on the face's boundary
                ! check if it is entering or exiting
                imax = maxloc(abs(uvtmp),1)
                if ( abs(uvtmp(imax)) > 1._fp - EPSuv ) then
                   imin = 1 + mod(imax,2)
                   if ( sens == 1 ) vec = -vec ! the face is on the left side
                   if ( abs(uvtmp(imin)) > 1._fp - EPSuv ) then
                      ! point on a 'natural' corner
                      nor = [vec(2), -vec(1)] ! outgoing normal
                      det = dot_product(nor,duvtmp) 
                      IF ( DEBUG ) PRINT *,'CORNER',det, ', outgoing normal =',nor
                      change_face = ( det > EPSmath )
                   else
                      ! point along a 'natural' border
                      IF ( DEBUG ) PRINT *,'BORDER',duvtmp(imax)*uvtmp(imax)
                      change_face = ( duvtmp(imax)*uvtmp(imax) > EPSmath )
                   end if
                else
                   ...
                   if ( stat < 2 ) then
                      ...
                      inside = ( h < -EPSmath )
                      if ( stat == 1 ) inside = .not.inside ! face boundary locally concave
                      IF ( DEBUG) THEN
                         PRINT *,'AO =',AO,', |RAD| =',NORM2(AO)
                         PRINT *,'|UV + DUV - O|/|RAD| =',SQRT(sum((duvtmp - ao)**2) / sum(ao**2))
                         PRINT *,'H/L =',(NORM2(duvtmp - ao) - NORM2(ao)) / NORM2(DUVTMP)
                         PRINT *,'L/R =',SQRT(SUM(DUVTMP**2) / SUM(AO**2))
                         PRINT *,'H/R =',(NORM2(duvtmp - ao) - NORM2(ao)) / NORM2(AO)
                      END IF
                      if ( .not.inside ) then
                         if ( h < 0.05_fp*norm2(duvtmp) .and. sum(duvtmp**2) < 0.1_fp*sum(ao**2) ) then
                            ! displacement is nearly tangent to the circular approximation of the boundary
                            IF ( .TRUE. ) THEN
                               ...
                               if ( stat > 0 ) then
                                  ...
                                  CALL WRITE_POLYNOMIAL(curve%surf(1)%ptr%x, 'Jouke/debug_relaxtng_surf1.cheb')
                                  CALL WRITE_POLYNOMIAL(curve%surf(2)%ptr%x, 'Jouke/debug_relaxtng_surf2.cheb')
                                  PRINT *,'UVinter =',(1._fp - w)*polyline%uv(:,:,i1) + w*polyline%uv(:,:,i2)
                                  PAUSE 
                               end if
                               ! add polyline point
                               ...
                               inside = .true.
                            ELSE
                               duvtmp = duvtmp - 1.01_fp * h * (duvtmp - ao) / (norm2(ao) + h)
                               IF ( DEBUG ) PRINT *,'MODIF DUVTMP =',duvtmp
                               inside = .true.
                            END IF
                         end if
                      else
                         ! inside: intersection is a false-positive
                         wtmp = lamb(2,iinter)!dot_product(duvtmp, vec) / sum(vec**2)
                         IF ( DEBUG ) PRINT *,'WTMP =',WTMP,', *|VEC| =',wtmp*norm2(vec)
                         if ( is_in_open_interval(wtmp*norm2(vec), 0._fp, norm2(vec), EPSuv) ) then
                           ! if ( w > 0.5_fp ) wtmp = 1._fp - wtmp
                            uvinter = (1._fp - wtmp)*polyline%uv(:,:,i1) + wtmp*polyline%uv(:,:,i2)
                            IF ( DEBUG ) THEN
                               PRINT *,'UVinter =',uvinter
