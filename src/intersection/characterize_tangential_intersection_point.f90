subroutine characterize_tangential_intersection_point( &
  ...
  real(kind=MATHpr),            intent(in)  :: s1(3,2,2)
  real(kind=MATHpr),            intent(in)  :: n(3,2)
  real(kind=MATHpr),            intent(out) :: xyz_s(3,2)
  integer,                      intent(out) :: stat
  real(kind=MATHpr)                         :: s2(3)
  real(kind=MATHpr)                         :: aux(3,2), A(2,2), LMN(3,2), lambda, B(3)
  real(kind=MATHpr)                         :: discr, w(2)
  integer                                   :: isurf, ivar
  ...
  do isurf = 1,2
     do ivar = 1,3
        call evald2( s2, surf(isurf)%ptr, uv(:,isurf), ivar )
  ...
  if ( all( abs(B) < MATHeps ) ) then
     ! higher order contact point (xyz_s cannot be determined)
     stat = 3
     return
     
  elseif ( abs( discr ) < MATHeps ) then
  ...
  elseif ( discr < 0._MATHpr ) then
     ...
     stat = 1
     w = real( [-1,1], kind=MATHpr ) * sqrt( discr )
     if ( abs(B(1)) > abs(B(3)) ) then
        w = (w - B(2)) / B(1)
