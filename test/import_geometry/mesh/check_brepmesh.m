...
% l = find(idstyp(:,1) == 12 & idstyp(:,2) == 1);% - 15;%[1666,2773];
% plot3(xyz(l,1), xyz(l,2), xyz(l,3), '.', 'color', 'g', 'markersize', 6);
...
if 1
    clpth = 0.6 * colorcet( 'R3', 'N', max(2,npths) );
    for i = 1:npths
        l = pth(i).verts;
...
%         plot3(xyz(l,1), xyz(l,2), xyz(l,3), 'o-', 'color', clp(i,:), 'linewidth', 2);
        ...
        b = xyz(l(end),:);
        v = b - a;
        quiver3(a(1), a(2), a(3), v(1), v(2), v(3), 0, 'color', clpth(i,:), 'linewidth', 2, 'maxheadsize', 20);
    end
end
