program demo_enright
  ...
  real, parameter                 :: ftsnap(4) = [0., 1./8., 1./4., 1./2.]
  ...
  integer                         :: itsnap(size(ftsnap)), isnap, jsnap, i, j, fid
  ...
  call init_from_surfaces( &
          '../cases/enright/enright_demo.opt', &
          options, &
          surf, &
          nsurf, &
          interdata, &
  ...
  itsnap = nint(3.0*ftsnap/options%timestep)
...
! write surfaces
           do isurf = 1,nsurf
              ...
              call write_polynomial( &
                   surf(isurf)%x, &
                   'demo_enright/snap_' // strnum // 'c_' // strnum2 // '.cheb' )
           end do
           ...
           IF ( .FALSE. ) THEN
              ...
              call write_obj_mesh( &
                   mesh, &
                   'demo_enright/snap_'//strnum//'.obj' )
           END IF
        
           ! write BRep vertices and edges
           call get_free_unit(fid)
           open(unit=fid, file='demo_enright/verts_'//strnum//'.dat', action='write')
           do i = 1,brep%nv
              write (fid,*) brep%verts(i)%point%xyz
           end do
           close(fid)

           open(unit=fid, file='demo_enright/edges_'//strnum//'.dat', action='write')
           write (fid,*) brep%ne
           do i = 1,brep%ne
