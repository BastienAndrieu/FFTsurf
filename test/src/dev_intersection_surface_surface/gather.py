VERBOSE = True

progname = 'dev_intersection_surface_surface'
f = open( '../' + progname + '.f90', 'w' )
f.write('program ' + progname + '\n')

f.writelines( [l for l in open('main.f90').readlines()] )


#f.write('\n\ncontains\n')

listfiles_simple_surface = ['intersect_border_surface.f90',
                            'append_vector.f90',
                            'intersect_curve_surface_elsewhere.f90',
                            'rearrange_for_separability_test.f90', 
                            'newton_curve_surface.f90',
                            'intersect_curve_surface.f90',
                            'inherit_points.f90',
                            'intersect_simple_surfaces.f90',
                            'classify_border_surface_intersection_point.f90',
                            'classify_endpoint.f90',
                            'diffgeom_intersection_curve.f90',
                            'characterize_tangential_intersection_point.f90',
                            'add_intersection_point.f90',
                            'print_intersection_point.f90',
                            'reallocate_polyline.f90',
                            'insert_polyline_point.f90',
                            'trace_intersection_polyline.f90',
                            'newton_intersection_polyline.f90']

listfiles = ['intersect_surface_surface.f90',
             'hohmeyer_loop_detection.f90', 
             'find_collineal_corners.f90',
             'find_collineal_points.f90',
             'intersect_gaussmaps_elsewhere.f90',
             'free_intersection_data.f90',
             'intersect_surface_surface_elsewhere.f90',
             'rearrange_for_separability_test_gaussmap.f90']

for filename in listfiles_simple_surface:
    if VERBOSE:
        print '\t * <---', filename
    f.write('\n\n\n\n\n')
    f.writelines( [l for l in open( '../dev_intersection_simple_surface/' + filename).readlines()] )


for filename in listfiles:
    if VERBOSE:
        print '\t * <---', filename
    f.write('\n\n\n\n\n')
    f.writelines( [l for l in open(filename).readlines()] )

f.write('end program ' + progname + '\n')

f.close()
