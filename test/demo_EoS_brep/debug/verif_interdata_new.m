...
files = {files_s.name, files_e.name, files_v.name}';
nsurf = numel(files);
...
points = importdata( 'intersection_points_new.dat' );
np = size(points,1);

fid = fopen( 'intersection_curves_new.dat', 'r' );
...
if ~isempty(points)
    lp = 1:np;%
    plot3( points(lp,1), points(lp,2), points(lp,3), '.', 'color', cli, 'markersize', 10 );
end
...
if ~isempty(dups)
    plot3(points(dups(1,:),1), points(dups(1,:),2), points(dups(1,:),3), 'ro');
end
