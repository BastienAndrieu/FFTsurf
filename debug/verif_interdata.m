...
for isurf = listsurf
    ...
    c = readCoeffs2( sprintf('../cases/jouke/output/debug/instant_058c_%3.3d.cheb', isurf) );
...
for ic = 1:nc
    ...
    %     plot3( curves(ic).xyz(:,1), curves(ic).xyz(:,2), curves(ic).xyz(:,3), ...
    %         '.-', 'color', cl(ic,:), 'markersize', 4 );
    %     m = round(0.5*size(curves(ic).xyz,1));
    %     text(curves(ic).xyz(m,1), curves(ic).xyz(m,2), curves(ic).xyz(m,3), ...
    %         num2str(ic), 'color', cl(ic,:), 'backgroundcolor', 'w');
    e = curves(ic).isplit(:,1)';
    plot3( points(e,1), points(e,2), points(e,3), '.', 'color', cli, 'markersize', 10 );
    %     for j = e
    %         text(points(j,1), points(j,2), points(j,3), ...
    %             num2str(j), 'color', 'r');
    %     end
end
