...
fprintf('%d verts, %d edges, %d faces\n', nv, ne, nf);
...
for ihe = [11,13,14]%1:nhe
    for i = 1:hyperedge(ihe).ne
        ...
        for v = hyperedge(ihe).verts
            text(vxyz(v,1), vxyz(v,2), vxyz(v,3), num2str(v), 'backgroundcolor', 'w');
        end
    end
end
v = 9;
text(vxyz(v,1), vxyz(v,2), vxyz(v,3), num2str(v), 'color', 'r', 'backgroundcolor', 'w');
